...
loc_vars = function(self, info_queue, card)
    ...
    return { vars = {card.ability.extra.xmult_mod, card.ability.extra.food_used_total}, key = self.key..'small' }
    end
  
    if card.ability.extra.food_used_total >= 7 then
    return { vars = {card.ability.extra.xmult_mod, card.ability.extra.food_used_total}, key = self.key..'big' }
    end
