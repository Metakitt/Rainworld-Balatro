...
config = { extra = { attuned = false, discards = 25, saint_discards = 25, recharge = false, xmult = 1, recharging = 'No.' }, name = 'Saint', blessed = false, slugcat = true },
	
loc_vars = function(self, info_queue, card)
    if card.ability.extra.attuned == true then
    return { vars = {card.ability.extra.saint_discards, card.ability.extra.recharging}, key = self.key..'attuned' }
    end
  
    if card.ability.extra.attuned == false then
    return { vars = {card.ability.extra.saint_discards, card.ability.extra.recharging}, key = self.key..'neutral' }
    end
...
calculate = function(self, card, context)
   ...
   if context.discard and card.ability.name == 'Saint' and card.ability.extra.attuned == false and card.ability.extra.recharge == false and not context.blueprint then
   card.children.center:set_sprite_pos({x = 7, y = 0})
	...
	if context.discard and card.ability.name == 'Saint' and card.ability.extra.attuned == false and card.ability.extra.recharge == true and not context.blueprint then
	card.children.center:set_sprite_pos({x = 7, y = 0})
	...
	if context.setting_blind and G.GAME.blind.boss and card.ability.extra.attuned == true and card.ability.extra.recharge == false and not context.blueprint then
	local jokers = {}
    ...
    local joker_to_destroy = #destructable_jokers > 0 and pseudorandom_element(destructable_jokers, pseudoseed('explode')) or nil
